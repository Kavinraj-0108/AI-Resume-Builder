...
const Dashboard = () => {
  ...
  const colors = ["#9333ea", "#d97706", "#dc2636", "#0284c7", "#16a34a"]
  ...
  const editTitle = async (e) => {
    try {
      ...
      console.log("Updating resume title:", { resumeId: editResumeId, newTitle: title });
      ...
      // Reload all resumes from server to ensure data is fresh
    ...
    <div>
      <div className='max-w-7xl mx-auto px-4 py-8'>
        ...
        <p className='text-2xl font-medium mb-6 bg-gradient-to-r from-slate-600 to-slate-700
          bg-clip-text text-transparent sm:hidden'>Welcome , Kavinraj M</p>
        ...
        <div className='grid grid-cols-2 sm:flex flex-wrap gap-4'>
          {allResumes.map((resume, index) => {
            ...
            return (
              <button onClick={() => navigate(`/app/builder/${resume._id}`)} key={index} className='relative w-full sm:max-w-36 h-48 flex
          ...
          <form onSubmit={createResume} onClick={() => setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50
            ...
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md 
              ...
              <button className='w-full py-2 bg-green-600 text-white rounded
                hover:bg-green-700 transition-colors'>Create a Resume</button>
          ...
          <form onSubmit={uploadResume} onClick={() => setShowUploadResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50
            ...
            <div onClick={e => e.stopPropagation()} className='relative bg-slate-50 border shadow-md 
              ...
              <button className='w-full py-2 bg-green-600 text-white rounded
                ...
                {isLoading && <LoaderCircleIcon className='size-4 text-white' />}
